...
export default {
  ...
  methods: {
    ...
    async getChainProperties(force = false) {
      ...
      this.chain.account_creation_fee = properties.account_creation_fee;
    ...
    calculateFee(operation) {
      let size = JSON.stringify(operation).replace(/[\[\]\,\"]/g, '')
        .length;
      let operationFlatFee = parseFloat(this.$store.state.chain.operation_flat_fee );
      let bw_fee = Math.max(
        0.001,
        ((size / 1024) * parseFloat(this.$store.state.chain.bandwidth_kbytes_fee)).toFixed(3)
      );
      return (operationFlatFee + bw_fee).toFixed(3)+ ' ' + Config.STEEM;
    }
  }
}
