  ...
  <div class="account">
    ...
    <div v-if="this.exists.account">
      <div
        class="profile"
        :style="
          this.account.cover_image == ''
            ? 'background-color: black;'
            : 'background-image: url(' + this.account.cover_image + ');'
        "
      >
        <div>
          <div
            class="image"
            :style="
              'background-image: url(' + this.account.profile_image + ');'
            "
          ></div>
          <div class="name">
            <h1>
              <strong>@{{ this.account.name }}</strong>
            </h1>
          </div>
        </div>
        <div>
          View on:
          <a :href="BLOG + '@' + this.account.name"> Blurt.World </a>
          |
          <a :href="BLURT_BUZZ + '@' + this.account.name"> Blurt.buzz </a>
        </div>
      </div>
      <div class="container">
        <div class="row">
          ...
          <card-data :data="this.witness.sbd_exchange_rate"></card-data>           -->
            ...
            <card-data
              :data="this.account.witness_votes"
              typeCard="witnesses"
              :link="true"
            ></card-data>
            <h2>Authorities</h2>
          ...
          <div class="loader"></div>
            ...
            <div class="center">
              <div v-for="(p, key, index) in pages" class="page">
                <span v-if="p.link"
                  ><router-link :to="EXPLORER + p.link">{{
                    p.text
                  }}</router-link></span
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      result[0].profile_image = Utils.extractUrlProfileImage(result[0].name);
      result[0].cover_image = Utils.extractUrlCoverImage(
      ...
      var delegated =
        parseFloat(this.account.received_vesting_shares) -
        parseFloat(this.account.delegated_vesting_shares);
      this.accountGenerals = {
        ...
        "Blurt Power":
          this.vests2sp(this.account.vesting_shares) +
          " (" +
          (delegated > 0 ? "+" : "") +
          this.vests2sp(delegated) +
          ")",
      };
      ...
      var result = await this.steem_database_call("get_witness_by_account", [
        name,
      ]);
    ...
    arrayAuthorities: function (auth) {
      var array = [];
      for (var i = 0; i < auth.key_auths.length; i++)
        array.push(auth.key_auths[i][0]);
      for (var i = 0; i < auth.account_auths.length; i++)
        array.push({
