<template>
  <div class="votes">
    <div class="title row">
      ...
      <div class="col-2">Curation Weight</div>
      <div class="col-2">Rshares</div>
    ...
    <div class="data row" v-for="(v, key, index) in votes">
      ...
      <div class="col-1">{{ v.vote_weight }}</div>
      <div class="col-1">{{ v.vote_value }}</div>
      <div class="col-2">{{ v.curation }}</div>
      <div class="col-2">{{ v.weight }}</div>
      <div class="col-2">{{ v.rshares }}</div>
...
export default {
  ...
  methods: {
    ...
    votesConvergentLinear() {
      var sortByTime_ascending = (a, b) => {
        return new Date(a.time) - new Date(b.time);
      };
      var sortByTime_descending = (a, b) => {
        return new Date(a.time) - new Date(b.time);
      };

      var votes_aux = this.data.sort(sortByTime_ascending);
      var total_weight = 0;
      var total_rshares = votes_aux.reduce(function (t, v) {
        return t + parseInt(v.rshares);
      }, 0);
      ...
      if (this.payout.old_post) {
        ...
        total_weight = votes_aux.reduce(function (t, v) {
          return t + v.weight;
        }, 0);
      } else {
        total_payout_curator_sbd = 0.5 * total_payout_sbd;
      ...
      for (var i = 0; i < votes_aux.length; i++) {
        ...
        v.vote_value =
          ((new_claims - old_claims) * this.chain.steem_per_rshare).toFixed(3) +
          " " +
          Config.SP;
        v.vote_value_before =
          (new_claims * this.chain.steem_per_rshare).toFixed(3) +
          " " +
          Config.SP;
        v.vote_weight = (v.percent / 100).toFixed(2) + "%";
        if (this.payout.old_post) {
          v.curation =
            ((total_payout_curator_sbd * v.weight) / total_weight).toFixed(3) +
            " " +
            Config.SP;
        } else {
          if (this.chain.feed_price >= 0) {
            v.curation =
              (
                (total_payout_curator_sbd * v.weight) /
                total_weight /
            ...
            v.curation =
              ((total_payout_curator_sbd * v.weight) / total_weight).toFixed(
                3
              ) +
              " " +
              Config.SP;
    ...
    votesLinear() {
      var votes_aux = this.data.sort(function (a, b) {
        return parseInt(b.rshares) - parseInt(a.rshares);
      });
      var total_weight = 0;
      var total_rshares = votes_aux.reduce(function (t, v) {
        return t + parseInt(v.rshares);
      ...
      for (var i = 0; i < votes_aux.length; i++) {
        ...
        v.vote_weight = (v.percent / 100).toFixed(2) + "%";
        v.vote_value =
          ((total_payout_sbd * parseInt(v.rshares)) / total_rshares).toFixed(
            3
          ) +
          " " +
          Config.SP;
