        ...
        <div class="col-md-3">
          <div v-if="this.exists.globals">
            <div class="card">
              ...
              <br />
              {{this.chain.current_supply}}
            </div>
            <div class="card">
              <div class="title">BLURT Price</div>
              <br />
              Price: ${{Number(this.price.price_usd).toFixed(3)}}
              <br />
              High: {{this.price.high}} BTC/BLURT
              <br />
              Low: {{this.price.low}} BTC/BLURT
              <br />
              Change: {{this.price.change}}%
              <br />
            </div>
            <div class="card">
              <div class="title">Transaction Fees</div>
              <br />
              Operation Flat Fee: {{this.chain.operation_flat_fee}}
              <br />
              Bandwidth Fee/KByte: {{this.chain.bandwidth_kbytes_fee}}
              <br />
               Account Creation Fee: {{this.chain.account_creation_fee}}
               <br />
            </div>
            <div class="card">
              <div class="title">Inflation</div>
              <br />
              Annual rate: {{this.chain.current_inflation_rate}}
              <br />
            </div>
            <div class="card">
              <div class="title">Staked Supply</div>
              <br />
              Fund: {{this.chain.total_vesting_fund_blurt}}
              <br />
              Shares: {{this.chain.total_vesting_shares}}
              <br />
            </div>
            <!-- <div class="card"> 
          ...
          <div v-if="this.exists.globals && this.exists.reward">
            <div class="card">
              ...
              <br />
              {{this.chain.reward_balance}} {{this.STEEM_SYMBOL}}
              <br />for next 15 days
              <br />
              <hr />
              ...
              <br />
              <hr />
              recent claims {{this.chain.recent_claims}}
              <br />
              <a
                href="https://steemit.com/@jga/complete-guide-to-understand-rewards-in-hf21-part-1"
              >gap</a>
              : {{this.chain.gap.toFixed(3)}} {{this.STEEM_SYMBOL}}
        ...
        <div class="col-md-7">
          <div>
            <b-tabs card>
                    ...
                    >
                      <div class="block-left">
                        ...
                        <span v-if="b.loaded">
                          ...
                          <span
                            v-if="b.size_posts>0"
                          >({{b.size_posts}} posts)</span>
                        </span>
                        <span v-else>loading...</span>
                      </div>
                      <div class="block-right">
                        <span class="small">witness</span>
                        <br />
              ...
              <b-tab title="Last Transactions">
                <div v-if="transactionsSize > 0">
                  <transition-group name="list-blocks" tag="div" class="block-group">
                    <div v-for="(b,key,index) in lastBlocks" :key="b.block_num">
                      <div v-for="transaction in b.transactions" :key="transaction.transaction_id">
                        <trx :tx="transaction"></trx>
                      </div>
                    </div>
                  </transition-group>
                </div>
                <div v-else>
                  <div id="divSpinner" class="spinner loading">
                    <span class="text">Please wait for the block stream</span>
                  </div>
                </div>
              </b-tab>
            </b-tabs>
        ...
        <div class="col-md-2">
          <div v-if="exists.schedule" class="schedule">
            ...
            <transition-group name="list-schedule" tag="div">
              <div v-for="(wit,key,index) in schedule" :key="wit" class="list-schedule-item">{{wit}}</div>
            </transition-group>
          </div>
          <div v-else>
            <div class="loader"></div>
...
export default {
  ...
  methods: {
    async getWitnessSchedule() {
      ...
      var witness_schedule = await this.steem_database_call(
      ...
      if (this.lastBlocks.length > 0)
        current_witness = this.lastBlocks[0].witness;
    ...
    getExtendedChainProperties() {
      ...
      var current_inflation_rate = Utils.getInflationRate(
        this.chain.head_block_number
      );
      this.chain.current_inflation_rate = current_inflation_rate / 100 + "%";
      this.chain.new_steem_per_day =
        (
          (Config.STEEM_BLOCKS_PER_DAY *
            parseFloat(this.chain.virtual_supply) *
            (current_inflation_rate / 10000)) /
          Config.STEEM_BLOCKS_PER_YEAR
        ).toFixed(3) +
        " " +
        Config.STEEM;
      this.chain.sp_percent =
        (parseFloat(this.chain.total_vesting_fund_steem) * 100) /
        parseFloat(this.chain.virtual_supply);
      this.chain.sbd_percent =
        ((parseFloat(this.chain.current_sbd_supply) / this.chain.feed_price) *
          100) /
        parseFloat(this.chain.virtual_supply);
      this.chain.sbd_per_rshare =
        this.chain.steem_per_rshare * this.chain.feed_price;
      this.chain.reward_percent =
        (parseFloat(this.chain.reward_balance) * 100) /
        parseFloat(this.chain.virtual_supply);
      this.chain.reward_balance_day =
        (parseFloat(this.chain.reward_balance) / 15).toFixed(3) +
        " " +
        Config.STEEM;
      this.chain.vote_value_1000_sp =
        (((1 / 50) * 1000 * this.chain.steem_per_rshare * 1e12) /
          this.chain.steem_per_mvests) *
        this.chain.feed_price;
    },
    ...
    fetchBlocks() {
      ...
      newBlocks.forEach(function (num) {
        var b = {
          block_num: num,
          timestamp_milis:
            last_block_recorded.timestamp_milis +
            (num - last_block_recorded.block_num) * 3000,
          time_ago: "",
          size_txs: 0,
          size_posts: 0,
          witness: "",
        ...
        if (self.lastBlocks.length > SIZE_BLOCKS) {
          self.lastBlocks.pop();
          self.transactionsSize = 0;
          for (let block of self.lastBlocks) {
            self.transactionsSize += block.size_txs;
          }
        }
    ...
    getBlock(b) {
      let self = this;
      self
        .steem_database_call("get_block", [b.block_num])
        .then(function (resultBlock) {
          if (!resultBlock) {
            //Block still does not exist.
          ...
          b.size_posts = resultBlock.transactions.filter(function (tx) {
            return (
              tx.operations[0][0] == "comment" &&
              tx.operations[0][1].parent_author == ""
            );
          }).length;
          b.transactions = resultBlock.transactions;
          b.timestamp_milis = new Date(resultBlock.timestamp + "Z").getTime();
          b.witness = resultBlock.witness;
          b.loaded = true;
          var pos = self.lastBlocks.findIndex(function (blk) {
            return blk.block_num == b.block_num;
          });
          if (pos >= 0) {
            self.$set(self.lastBlocks, pos, b);
            if (pos == 0 && b.witness == self.schedule[0])
              self.schedule.shift();
          }
        })
