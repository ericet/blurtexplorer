        ...
        <sorted-table :values="richlist">
          <thead>
            <tr class="table-primary">
              ...
              <th scope="col">
                <sort-link name="totalAccountBp">Total Account SP</sort-link>
              </th>
          ...
          <tbody slot="body" slot-scope="sort">
            <tr v-for="rich in sort.values" :key="rich.name">
              ...
              <td>{{ rich.balance }} STEEM</td>
              <td>{{ rich.bp }} SP</td>
              <td>{{ rich.delegationOut }} SP</td>
              <td>{{ rich.delegationIn }} SP</td>
              <td v-if="rich.diff===''">{{ rich.withdrawRate }} SP</td>
              <td v-else>{{ rich.withdrawRate }} SP in {{rich.diff}}</td>
              <td>{{ rich.totalAccountBp }} SP</td>
            </tr>
          </tbody>
        </sorted-table>
      </div>
  ...
  methods: {
    async getRichlist() {
      try {
        ...
        const response = await axios.get("https://api.blurt.buzz/steem-richlist");
        if (response.status === 200) {
          ...
          for (let account of accounts) {
            ...
            this.richlist.push({
              num: count++,
              name: account.name,
              imgUrl: `https://https://images.blurt.blog/u/${account.name}/avatar`,
